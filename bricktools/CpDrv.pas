...
uses
  // Delphi units
  Windows, Messages, SysUtils, Classes, Forms, TOWERAPI
  // ComDrv32 units
  ;
...
type
  ...
  TPortNumber = ( pnCustom,
                  ...
                  pnCOM8, pnUSB1, pnUSB2, pnUSB3, pnUSB4 );
  // Data bits ( 5, 6, 7, 8 )
...
type
  ...
  protected
    ...
    procedure SetPort( Value: TPortNumber ); virtual;
    // Sets the port name
    procedure SetPortName( Value: string ); virtual;
    // Selects the baud rate
    procedure SetBaudRate( Value: TBaudRate ); virtual;
    // Selects the baud rate ( actual baud rate value )
    procedure SetBaudRateValue( Value: DWORD ); virtual;
    ...
    function ApplyCOMSettings: boolean; virtual;
    // Polling proc
    procedure TimerWndProc( var msg: TMessage );
    // Applies comm timouts
    procedure ApplyCOMTimeouts; virtual;
    // get # of bytes in the queue
    function GetBytesInQueue : DWORD; virtual;
    // set the port name based on the port
    procedure UpdatePortName; virtual;
    // flush the output buffer
    procedure FlushWrite; virtual;
    // drain the RX buffer;
    procedure DrainRXBuffer; virtual;
    // read from the handle
    function DoReadFromHandle(DataPtr: Pointer; const nToRead : Cardinal;
      var nRead : Cardinal; const nMax : Cardinal) : Boolean; virtual;
    // internal send data routine
    function InternalSendDataEx( DataPtr: pchar; DataSize, Timeout: DWORD ): DWORD;
    // keep trying to read data?
    function KeepOnReading(nRead : Integer) : Boolean; virtual;
    function SkipTimeoutCheck : Boolean; virtual;
  public
    // Constructor
    ...
    function GetLineStatus: TLineStatusSet; virtual;
    ...
    function FlushBuffers( inBuf, outBuf: boolean ): boolean; virtual;
    // Returns number of received bytes in the RX buffer
    function CountRX: integer; virtual;
    // Returns the output buffer free space or 65535 if not connected
    function OutFreeSpace: word; virtual;
    // Sends binary data
    ...
    function SendDataEx( DataPtr: pchar; DataSize, Timeout: DWORD ): DWORD; virtual;
    ...
    function ReadData( DataPtr: Pointer; MaxDataSize: DWORD ): DWORD; virtual;
    ...
    procedure ToggleDTR( onOff: boolean ); virtual;
    // Set RTS line high (onOff=TRUE) or low (onOff=FALSE).
    // You must not use HW handshaking.
    procedure ToggleRTS( onOff: boolean ); virtual;
  ...
  TUSBPortDriver = class(TCommPortDriver)
  private
    fReadICTimeout: Word;
    fWriteTimeout: Word;
    fReadFirstTimeout: Word;
  ...
  protected
    procedure SetBaudRate( Value: TBaudRate ); override;
    procedure SetBaudRateValue( Value: DWORD ); override;
    function ApplyCOMSettings: boolean; override;
    procedure ApplyCOMTimeouts; override;
    function GetBytesInQueue : DWORD; override;
    procedure UpdatePortName; override;
    procedure FlushWrite; override;
    function DoReadFromHandle(DataPtr: Pointer; const nToRead : Cardinal;
      var nRead : Cardinal; const nMax : Cardinal) : Boolean; override;
    function KeepOnReading(nRead : Integer) : Boolean; override;
    function SkipTimeoutCheck : Boolean; override;
  public
    constructor Create( AOwner: TComponent ); override;
    function CountRX: integer; override;
    function FlushBuffers( inBuf, outBuf: boolean ): boolean; override;
    function GetLineStatus: TLineStatusSet; override;
    function OutFreeSpace: word; override;
    procedure ToggleDTR( onOff: boolean ); override;
    procedure ToggleRTS( onOff: boolean ); override;
    function PortIsUSB : Boolean;
    property ReadFirstTimeout : Word read fReadFirstTimeout write SetReadFirstTimeout;
    property ReadICTimeout : Word read fReadICTimeout write SetReadICTimeout;
    property WriteTimeout : Word read fWriteTimeout write SetWriteTimeout;
  end;
...
{$ENDIF}
{$IFDEF LINUX}
    FNotifyWnd := WinUtils.AllocateHWnd(TimerWndProc);
{$ENDIF}
end;
...
{$ENDIF}
{$IFDEF LINUX}
    WinUtils.DeallocateHWnd(FNotifyWnd);
{$ENDIF}
...
begin
  Result := InternalSendDataEx(DataPtr, DataSize, Timeout);
end;
...
// Reads binary data. Returns number of bytes read
function TCommPortDriver.KeepOnReading(nRead: Integer): Boolean;
begin
  Result := nRead > 0;
end;

function TCommPortDriver.ReadData( DataPtr: Pointer; MaxDataSize: DWORD ): DWORD;
...
begin
  ...
  try
    ...
    begin
      ...
      if KeepOnReading(nToRead) then
      begin
        // Don't read more bytes than we actually have to read
        if nToRead > MaxDataSize then
          nToRead := MaxDataSize;
        // Read
        DoReadFromHandle(DataPtr, nToRead, nRead, MaxDataSize);
...
begin
  ...
  begin
    ...
    begin
      ...
      if bytesInQue >= DWORD(FPacketSize) then
      begin
        repeat
          // Read the packet and pass it to the app
          nRead := 0;
          if DoReadFromHandle(FTempInBuffer, FPacketSize, nRead, InBufSize) then
//          if ReadFile( FHandle, FTempInBuffer^, FPacketSize, nRead, nil ) then
          ...
          bytesInQue := bytesInQue - WORD(FPacketSize);
          if bytesInQue = 0 then
            FFirstByteOfPacketTime := DWORD(-1);
        until bytesInQue < DWORD(FPacketSize);
        // Done
        exit;
      end;
      ...
      begin
        ...
        if DoReadFromHandle(FTempInBuffer, bytesInQue, nRead, InBufSize) then
//        if ReadFile( FHandle, FTempInBuffer^, bytesInQue, nRead, nil ) then
      ...
      // Start time
      if (bytesInQue > 0) and (FFirstByteOfPacketTime = DWORD(-1)) then
        FFirstByteOfPacketTime := GetTickCount;
      // Done
      exit;
    end;

    // Standard data handling (PacketSize < 0)
    nRead := 10;
    nToRead := bytesInQue;
    if (nToRead > 0) then
      if DoReadFromHandle(FTempInBuffer, nToRead, nRead, InBufSize) then
//      if ReadFile( FHandle, FTempInBuffer^, nToRead, nRead, nil ) then
...
procedure TCommPortDriver.ApplyCOMTimeouts;
...
function TCommPortDriver.GetBytesInQueue: DWORD;
var
  comStat: TCOMSTAT;
  dummy : DWORD;
begin
...
procedure TCommPortDriver.UpdatePortName;
begin
  // Update the port name
...
procedure TCommPortDriver.FlushWrite;
begin
  FlushFileBuffers(FHandle);
end;

procedure TCommPortDriver.DrainRXBuffer;
var
  fRxData : PChar;
...
begin
  fRxData := @tmpBuf[0];
  // drain rx buffer
  while ReadData(fRxData, 80) > 0 do
  begin
    // nothing
  end;
end;

function TCommPortDriver.DoReadFromHandle(DataPtr: Pointer;
  const nToRead : Cardinal; var nRead: Cardinal; const nMax : Cardinal): Boolean;
begin
  Result := ReadFile(FHandle, DataPtr^, nToRead, nRead, nil);
end;

function TCommPortDriver.InternalSendDataEx(DataPtr: pchar; DataSize,
  Timeout: DWORD): DWORD;
...
function TCommPortDriver.SkipTimeoutCheck: Boolean;
begin
  Result := True;
end;
...
function TUSBPortDriver.ApplyCOMSettings: boolean;
  function speed : Integer;
  begin
    if BaudRate = br4800 then
      Result := LT_SPEED_BPS_4800
    else
      Result := LT_SPEED_BPS_2400;
  end;
begin
  if PortIsUSB then
  begin
    ...
    begin
      Result := LEGOTowerSetLinkMode(fHandle, LT_LINK_IR);
      if Result then
        Result := LEGOTowerSetIRSpeed(fHandle, speed, speed);
      ...
      if Result then
        Result := LEGOTowerSetCarrierFrequency(fHandle, 38);
    end;
    // Flush buffers
    if Result then
      Result := FlushBuffers( true, true );
    ...
    Result := inherited ApplyCOMSettings;
end;

procedure TUSBPortDriver.ApplyCOMTimeouts;
begin
  if PortIsUSB then
  begin
    if LegoTowerApiLoaded then
      LEGOTowerSetTimeouts(FHandle, ReadFirstTimeout, ReadICTimeout, WriteTimeout);
  end
  else
    inherited ApplyCOMTimeouts;
end;

function TUSBPortDriver.CountRX: integer;
begin
  if PortIsUSB then
    Result := GetBytesInQueue
  else
    Result := inherited CountRX;
end;

constructor TUSBPortDriver.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  // USB1
  FPort          := pnUSB1;
  FPortName      := '\\.\LEGOTOWER1';
  // 2400 baud
  FBaudRate      := br2400;
  FBaudRateValue := BaudRateOf(br2400);
  // timeouts
  fReadFirstTimeout := 10;
  fReadICTimeout    := 0;
  fWriteTimeout     := 0;
end;

function TUSBPortDriver.DoReadFromHandle(DataPtr: Pointer;
  const nToRead: Cardinal; var nRead: Cardinal; const nMax : Cardinal): Boolean;
var
  p : Pointer;
  i : Cardinal;
begin
  ...
  begin
    i := 0;
    p := DataPtr;
    Result := ReadFile(FHandle, p^, nMax, nRead, nil);
    while Result and (nRead > 0) do
    begin
      p := PChar(p) + nRead;
      Inc(i, nRead);
      if i >= nMax then Break;
      Result := ReadFile(FHandle, p^, nMax, nRead, nil);
    end;
    nRead := i;
  end
  else
    Result := inherited DoReadFromHandle(DataPtr, nToRead, nRead, nMax);
end;

function TUSBPortDriver.FlushBuffers(inBuf, outBuf: boolean): boolean;
begin
  if PortIsUSB then
  begin
    Result := True;
    ...
    begin
      if inBuf then
        Result := LEGOTowerFlush(FHandle, LT_FLUSH_RX_BUFFER);
      if outBuf then
        Result := LEGOTowerFlush(FHandle, LT_FLUSH_TX_BUFFER);
    end;
    // Used by the RX packet mechanism
    if Result then
      FFirstByteOfPacketTime := DWORD(-1);
    ...
    Result := inherited FlushBuffers(inBuf, outBuf);
end;

procedure TUSBPortDriver.FlushWrite;
begin
  if PortIsUSB then
  begin
    if LegoTowerApiLoaded then
      LEGOTowerFlush(FHandle, LT_FLUSH_TX_BUFFER);
  end
  else
    inherited;
end;
var
  ResultArray : array[Boolean] of Integer = (1, 0);

function TUSBPortDriver.GetBytesInQueue: DWORD;
begin
  if PortIsUSB then
    Result := ResultArray[fInReadData]
  else
    Result := inherited GetBytesInQueue;
end;

function TUSBPortDriver.GetLineStatus: TLineStatusSet;
begin
  if PortIsUSB then
    Result := [lsCTS]
  else
    Result := inherited GetLineStatus;
end;

function TUSBPortDriver.KeepOnReading(nRead: Integer): Boolean;
begin
  if PortIsUSB then
    Result := true
  else
    Result := inherited KeepOnReading(nRead);
end;

function TUSBPortDriver.OutFreeSpace: word;
begin
  if PortIsUSB then
    Result := FOutBufSize
  else
    Result := inherited OutFreeSpace;
end;

function TUSBPortDriver.PortIsUSB: Boolean;
begin
  Result := FPort in [pnUSB1..pnUSB4];
end;

procedure TUSBPortDriver.SetBaudRate(Value: TBaudRate);
begin
  if not PortIsUSB or (Value in [br2400, br4800]) then
    inherited;
end;

procedure TUSBPortDriver.SetBaudRateValue(Value: DWORD);
begin
  if not PortIsUSB or (Value = CBR_2400) or (Value = CBR_4800) then
    inherited;
end;

procedure TUSBPortDriver.SetReadFirstTimeout(const Value: Word);
begin
  if not Connected then
    fReadFirstTimeout := Value;
end;

procedure TUSBPortDriver.SetReadICTimeout(const Value: Word);
begin
  if not Connected then
    fReadICTimeout := Value;
end;

procedure TUSBPortDriver.SetWriteTimeout(const Value: Word);
begin
  if not Connected then
    fWriteTimeout := Value;
end;

function TUSBPortDriver.SkipTimeoutCheck: Boolean;
begin
  if PortIsUSB then
    Result := false
  else
    Result := inherited SkipTimeoutCheck;
end;

procedure TUSBPortDriver.ToggleDTR(onOff: boolean);
begin
  if not PortIsUSB then
    inherited;
end;

procedure TUSBPortDriver.ToggleRTS(onOff: boolean);
begin
  if not PortIsUSB then
    inherited;
end;

procedure TUSBPortDriver.UpdatePortName;
begin
  // Update the port name
  if PortIsUSB then
    FPortName := Format( '\\.\LEGOTOWER%d', [ord(FPort)-ord(pnCOM8)] )
  else
    inherited;
end;
