...
$Id: SynEditAutoComplete.pas,v 1.1.1.1 2009/01/12 02:30:02 jhansen Exp $
...
begin
  ...
  TStringList(fAutoCompleteList).OnChange := {$IFDEF FPC}@{$ENDIF}CompletionListChanged;
...
begin
  ...
  for i := 0 to fEditors.Count - 1 do begin
    ...
    (O as TCustomSynEdit).RegisterCommandHandler({$IFDEF FPC}@{$ENDIF}SynEditCommandHandler, nil);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      fEditors.Delete(i);
      if ComponentState * [csDesigning, csLoading] = [] then
        AEditor.UnregisterCommandHandler({$IFDEF FPC}@{$ENDIF}SynEditCommandHandler);
    end;
  end;
  Result := False;
end;
...
procedure TCustomSynAutoComplete.SynEditCommandHandler(Sender: TObject;
  ...
  var Command: TSynEditorCommand; var AChar: {$IFDEF SYN_LAZARUS}TUTF8Char{$ELSE}Char{$ENDIF}; Data: pointer;
  HandlerData: pointer);
begin
