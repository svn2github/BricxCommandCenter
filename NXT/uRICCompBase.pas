...
//    procedure Draw(aPoint : TImgPoint; Vars : TRICVariables;  Options : Cardinal; aCanvas : TImgCanvas); override;
  ...
  protected
    ...
    procedure Clear;
    procedure DoImportKeyword(op: TRICSprite); virtual;
  public
...
constructor TRICCompBase.Create;
begin
  inherited;
...
destructor TRICCompBase.Destroy;
begin
...
{
procedure TRICCompBase.Draw(aPoint: TImgPoint; Vars: TRICVariables;
...
function TRICCompBase.GetAsText: string;
begin
  // make sure the stream matches the contents of fOperations
  SyncStreamToObjectList;
  Result := uRICCompBase.RICToText(fMS, fOperations, fCurFile);
end;

function TRICCompBase.GetOperation(Index: integer): TRICOpBase;
begin
  Result := TRICOpBase(fOperations[Index]);
end;

function TRICCompBase.GetOpCount: Integer;
begin
  Result := fOperations.Count;
end;

procedure TRICCompBase.LoadFromFile(const aFilename: string);
var
  Stream: TStream;
begin
  fCurFile := aFilename;
...
procedure TRICCompBase.LoadFromStream(aStream: TStream);
begin
  fMS.LoadFromStream(aStream);
  SyncObjectListToStream;
end;

class function TRICCompBase.RICToText(aStream: TStream; const aFilename : string): string;
begin
  with TRICCompBase.Create do
  try
    fCurFile := aFilename;
...
class function TRICCompBase.RICToText(const aFilename: string): string;
begin
  with TRICCompBase.Create do
  try
...
procedure TRICCompBase.SaveToFile(const aFilename: string);
var
  Stream: TStream;
begin
...
procedure TRICCompBase.SaveToStream(aStream: TStream);
begin
  SyncStreamToObjectlist;
  aStream.CopyFrom(fMS, 0);
end;

procedure TRICCompBase.SetAsText(const Value: string);
var
  SL : TStringList;
begin
  Clear;
...
procedure TRICCompBase.ReportProblem(const lineNo: integer; const fName,
  msg: string; err : boolean);
var
...
procedure TRICCompBase.IncLineNumber;
begin
  linenumber := linenumber + 1;
  inc(totallines);
end;

procedure TRICCompBase.GetCharX;
var
  bytesread : integer;
begin
...
procedure TRICCompBase.GetChar;
begin
...
function TRICCompBase.IsWhite(c: char): boolean;
begin
  Result := c in [' ', TAB, CR, LF, TOK_BLOCK_COMMENT, TOK_LINE_COMMENT];
end;

function TRICCompBase.IsAlpha(c: char): boolean;
begin
  Result := c in ['A'..'Z', 'a'..'z', '_'];
end;

function TRICCompBase.IsDigit(c: char): boolean;
begin
  Result := c in ['0'..'9'];
end;

function TRICCompBase.IsHex(c: char): boolean;
begin
  Result := IsDigit(c) or (c in ['a'..'f', 'A'..'F']);
end;

function TRICCompBase.IsAlNum(c: char): boolean;
begin
  Result := IsAlpha(c) or IsDigit(c) or (c = '.');
end;

procedure TRICCompBase.SkipCommentBlock;
begin
  repeat
    repeat
      GetCharX;
...
procedure TRICCompBase.SkipLine;
begin
  repeat
    GetCharX;
...
procedure TRICCompBase.SkipWhite;
begin
...
procedure TRICCompBase.AbortMsg(s: string);
begin
  ReportProblem(linenumber, CurrentFile, s, True);
end;

procedure TRICCompBase.Expected(s: string);
begin
  AbortMsg(Format(sExpectedString, [s]));
end;

procedure TRICCompBase.GetName;
begin
  SkipWhite;
...
procedure TRICCompBase.GetHexNum;
begin
  SkipWhite;
...
procedure TRICCompBase.GetString;
begin
  SkipWhite;
  GetChar; // skip the "
  Token := TOK_STRINGLIT;
...
procedure TRICCompBase.CheckBytesRead(const oldBytesRead: integer);
begin
...
procedure TRICCompBase.MatchString(x: string);
begin
  if Value <> x then Expected('''' + x + '''');
  Next;
end;

procedure TRICCompBase.CheckNumeric;
begin
  if not (Token in [TOK_NUM, TOK_HEX]) then Expected(sNumber);
end;

procedure TRICCompBase.CheckStringConst;
begin
  if (Token <> TOK_STRINGLIT) then
    Expected(sStringLiteral);
end;

procedure TRICCompBase.Semi;
begin
  MatchString(';');
end;

procedure TRICCompBase.OpenParen;
begin
  MatchString(TOK_OPENPAREN);
  inc(fParenDepth);
end;

procedure TRICCompBase.CloseParen;
begin
  dec(fParenDepth);
...
function TRICCompBase.StringToInt(const val : string) : integer;
begin
  Result := 0;
...
function TRICCompBase.ValueToInt : integer;
begin
  Result := StringToInt(Value);
end;

function TRICCompBase.ValueToSmallInt : SmallInt;
begin
  Result := SmallInt(ValueToInt);
end;

function TRICCompBase.ValueToWord : Word;
begin
  Result := Word(ValueToInt);
end;

function TRICCompBase.ProcessWordArg : Word;
begin
  Result := Word(ProcessArg);
end;

function TRICCompBase.ProcessArg : SmallInt;
var
  mapidx : SmallInt;
begin
  Result := 0;
...
procedure TRICCompBase.DoSprite;
var
  op : TRICSprite;
  sl : TStringList;
  i : integer;
...
procedure TRICCompBase.DoVarMap;
var
  op : TRICVarMap;
  ME : TMapElement;
begin
...
procedure TRICCompBase.DoCopyBits;
var
  op : TRICCopyBits;
  SR : TImgRect;
  DP : TImgPoint;
...
procedure TRICCompBase.DoPixel;
var
  op : TRICPixel;
  P : TImgPoint;
begin
...
procedure TRICCompBase.DoCircle;
var
  op : TRICCircle;
  P : TImgPoint;
begin
...
procedure TRICCompBase.DoEllipse;
var
  op : TRICEllipse;
  P : TImgPoint;
begin
...
procedure TRICCompBase.DoPolygon;
var
  op : TRICPolygon;
  PP : TPolyPoint;
begin
...
procedure TRICCompBase.DoNumBox;
var
  op : TRICNumBox;
  P : TImgPoint;
begin
...
procedure TRICCompBase.Statement;
begin
  case Token of
    TOK_DESC:       DoDesc;
    TOK_SPRITE:     DoSprite;
...
procedure TRICCompBase.ScriptCommands;
var
...
procedure TRICCompBase.InternalParseStream;
begin
  try
    fBadProgram     := False;
    fBytesRead      := 0;
...
procedure TRICCompBase.SyncObjectListToStream;
begin
  uRICCompBase.RICToText(fMS, fOperations, fCurFile);
end;

procedure TRICCompBase.SyncStreamToObjectList;
var
  i : integer;
begin             
  fMS.Clear;
...
procedure TRICCompBase.Parse(aStream: TStream);
begin
  Clear;
...
procedure TRICCompBase.Parse(aStrings: TStrings);
begin
  Clear;
...
procedure TRICCompBase.Parse(const aFilename: string);
var
  Stream : TFileStream;
begin
  Clear;
...
function TRICCompBase.SaveAsDataArray(const aLangName: TLangName; varname : string): string;
var
  tmp : string;
  i : integer;
begin
...
class function TRICCompBase.RICToDataArray(const aFilename, aVarName : string;
...
procedure TRICCompBase.CheckFirmwareVersion(const MinVer : word; const msg : string);
begin
  if FirmwareVersion < MinVer then
    AbortMsg(msg);
end;

procedure TRICCompBase.DoFontOut;
var
  op : TRICDescription;
begin
  // add a description opcode
...
procedure TRICCompBase.DoImportKeyword(op: TRICSprite);
begin
  // do nothing
end;
