...
type
  ...
  protected
    ...
    procedure DoArrayIndex(dt : char; aLHSName, aName : string);
...
begin
  ...
  else if IsUDT(dt) and (Token = '.') then
  begin
    // set the variable to the specified element from previous array
    udType := '';
    if IsUDT(ArrayBaseType(dt)) then
...
procedure TNXCComp.DoArrayIndex(dt : char; aLHSName, aName : string);
var
  oldType, tmpDT, lhsDT, rhsDT : char;
  tmp, udType, aval, idxName : string;
  AHV : TArrayHelperVar;
begin
  AHV := nil;
  Next;
  ...
  if Token = '[' then
  begin
    tmpDT := AddArrayDimension(dt);
//    if ArrayBaseType(tmpDT) <> ArrayBaseType(dt) then
...
(*
  ...
  if not TypesAreCompatible(lhsDT, rhsDT) then
    AbortMsg(sInvalidArrayExpr);
*)
  if IsArrayType(dt) or IsUDT(dt) or ({lhsDT}dt = TOK_STRINGDEF) then
