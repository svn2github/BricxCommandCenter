...
begin
  ...
  SetLength(tempdir,l);
  TempFileName := IncludeTrailingPathDelimiter(tempdir) + '~AWTemp.avi';
end;
...
var
  ExtBitmap : TObject;
  nstreams, i, AVIERR, refcount : integer;
  Streams : APAVISTREAM;
...
begin
    ...
    // If anything on the Bitmaps TList is not a bitmap, raise an error.
...
var
  ...
  Samples_Written, Bytes_Written : LongInt;
  AVIERR, i : integer;
begin
     ...
     try
        ...
        for i := 0 to Bitmaps.count - 1 do begin
           try
             ...
             ExtBitmap := Bitmaps[i] as TBitmap;
             if fStretch then
             ...
             AVIERR :=
                AVIStreamWrite(pStream, i, 1, BitmapBits, BitmapSize, AVIIF_KEYFRAME, Samples_Written, Bytes_Written);
             if AVIERR <> AVIERR_OK then
